...
import {
	ProductItemView,
	ProductItemModalView,
} from './components/view/partial/ProductCard';
...
const templates = {
	productsTemplate: ensureElement<HTMLTemplateElement>(
		SETTINGS.productCardMainTemplate
	),
	cardPreviewTemplate: ensureElement<HTMLTemplateElement>(
		SETTINGS.productCardPreviewTemplate
	),
	cardBasketTemplate: ensureElement<HTMLTemplateElement>(
		SETTINGS.productCardBasketTemplate
	),
	...
	contactsTemplate: ensureElement<HTMLTemplateElement>(
...
const components = {
	...
	success: new SuccessView(cloneTemplate(templates.successTemplate), {
		onClick: () => {
...
events.on(AppStateComponents.PRODUCT.CHANGE, () => {
	main.products = Array.from(app.products.values()).map((item) => {
		const product = new ProductItemView(
			cloneTemplate(templates.productsTemplate),
			{
...
events.on(AppStateComponents.PRODUCT.SELECT, (item: IProduct) => {
	main.locked = true;
	const product = new ProductItemModalView(
		cloneTemplate(templates.cardPreviewTemplate),
		{
			onClick: () => {
...
events.on(AppStateComponents.BASKET.OPEN, () => {
	...
	const basketContent = Array.from(app.basket.values()).map((item, index) => {
		const productInBasket = new ProductItemBasket(
...
events.on(AppStateComponents.ORDER.ERROR, (dataErr: IOrderMethod) => {
	...
	components.order.errors = Object.values(dataErr)
		.filter((errStr) => !!errStr)
		.join(' ');
});

// изменение order в AppState
events.on(
	AppStateComponents.ORDER.INPUT,
	(data: { field: keyof IOrderMethod; value: string }) => {
...
events.on(AppStateComponents.CONTACT.ERROR, (dataErr: IContacts) => {
	...
	components.contacts.errors = Object.values(dataErr)
		.filter((errStr) => !!errStr)
		.join(' ');
});
...
// изменение order в AppState
events.on(
	AppStateComponents.CONTACT.INPUT,
...
events.on(AppStateComponents.CONTACT.TO_SUCCESS, (answer: IOrderResult) => {
	modal.render({
		content: components.success.render({ description: answer.total }),
	});
});
